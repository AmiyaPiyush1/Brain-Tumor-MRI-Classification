...
def dice_score(predictions: torch.Tensor, targets: torch.Tensor,
    ...
    preds = (torch.sigmoid(predictions) > threshold).float().view(-1)
...
class SegmentationTrainer:
    """Training loop for brain tumor segmentation."""
    ...
    def train_fold(self, train_loader: DataLoader, val_loader: DataLoader,
                   fold: int, epochs: int = 50, patience: int = 10,
...
def train_segmentation_with_cross_validation(
    ...
    full_dataset = BRISCSegmentationDataset(image_dir, mask_dir, transform=None,
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        model, criterion = create_segmentation_model(
            num_classes=1, pretrained=True, classifier_weights_path=classifier_weights_path
        )
        ...
        trainer = SegmentationTrainer(model=model, criterion=criterion, optimizer=optimizer,
                                      scheduler=scheduler, device=device, use_amp=True)

        fold_history = trainer.train_fold(train_loader=train_loader, val_loader=val_loader,
                                          fold=fold, epochs=epochs, patience=patience,
        ...
        print(f"Fold {fold+1} — Dice: {final_dice:.4f}, IoU: {final_iou:.4f}")
