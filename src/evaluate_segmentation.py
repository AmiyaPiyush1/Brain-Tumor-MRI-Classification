...
def compute_pixel_metrics(pred_mask: np.ndarray, true_mask: np.ndarray,
                          smooth: float = 1e-6) -> Dict[str, float]:
    """Compute segmentation metrics for a single prediction-mask pair."""
    ...
    return {
        ...
        'recall':   float(recall),
        'f1':       float((2 * precision * recall) / (precision + recall + smooth)),
        'accuracy': float((tp + tn) / (tp + tn + fp + fn + smooth)),
        'tp': float(tp), 'fp': float(fp), 'fn': float(fn), 'tn': float(tn)
    }


class SegmentationEvaluator:
    """Evaluates a segmentation model across a full dataset."""
    ...
    def evaluate_model(self, model: torch.nn.Module, loader: DataLoader,
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="Evaluating"):
                ...
                for i in range(len(images)):
                    all_metrics.append(compute_pixel_metrics(pred_masks[i, 0], masks[i, 0]))
        ...
        return {'per_sample': all_metrics, 'aggregated': aggregated, 'n_samples': len(all_metrics)}
    ...
    def print_evaluation_report(self, results: Dict):
        ...
        labels = {
            'dice': 'Dice Score', 'iou': 'IoU (Jaccard)', 'precision': 'Precision',
            'recall': 'Recall (Sensitivity)', 'f1': 'F1 Score', 'accuracy': 'Pixel Accuracy'
        }
...
def visualize_predictions(model: torch.nn.Module, dataset,
    ...
    """
    Visualize segmentation predictions as a 4-column grid:
    ...
    for ax, title in zip(axes[0], ['Original MRI', 'Ground Truth Mask', 'Predicted Mask', 'Overlay']):
    ...
    for row, idx in enumerate(indices):
        ...
        with torch.no_grad():
            pred_prob = torch.sigmoid(model(image_tensor)).squeeze().cpu().numpy()
        pred_mask = (pred_prob > threshold).astype(np.uint8)

        image_np = sample['image'].permute(1, 2, 0).numpy()
        image_gray = (image_np * 0.5 + 0.5).clip(0, 1)[:, :, 0]

        overlay = np.stack([image_gray, image_gray, image_gray], axis=-1)
        overlay[pred_mask == 1] = [1.0, 0.2, 0.2]
...
def plot_training_history(history: Dict, save_path: Optional[str] = None):
    """Plot segmentation training curves (Loss, Dice, IoU)."""
    ...
    axes[1].set_title('Dice Score'); axes[1].set_xlabel('Epoch'); axes[1].set_ylim(0, 1)
    axes[1].legend(); axes[1].grid(True, alpha=0.3)

    axes[2].plot(history['val_iou'], label='Val IoU', color='seagreen')
    axes[2].set_title('IoU Score'); axes[2].set_xlabel('Epoch'); axes[2].set_ylim(0, 1)
